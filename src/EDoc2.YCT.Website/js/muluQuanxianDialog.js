...
$.widget( "ui.muluQuanxianDialog", {
	...
	_create: function() {
        ...
        this._grid.datagrid({
		    columns:[
			    {title: "成员名称", width: 100, field:"chengyuanMingcheng"},
			    {title: "一级答案", width: 80, field:"youYijiDaanQuanxian"},
			    {title: "二级答案", width: 80, field:"youErjiDaanQuanxian"},
			    {title: "三级答案", width: 80, field:"youSanjiDaanQuanxian"},
			    {title: "四级答案", width: 80, field:"youSijiDaanQuanxian"},
			    {title: "五级答案", width: 80, field:"youWujiDaanQuanxian"},
			    {title: "继承权限", width: 80, field:"strShiJichengDe"}
		    ],
            height: 300,
		    canSort: false,
		    singleSelect: true,
        ...
        this.element.dialog({autoOpen: false, width: 730, height: 500, modal: true});

        this._tianjiaMuluQuanxianDialog = self.element.find(".tianjiaMuluQuanxianDialog").eq(0);
        this._tianjiaMuluQuanxianDialog.tianjiaMuluQuanxianDialog({tianjiaHou: function(){
            self._shuaxinGrid();
        }});

        this._xiugaiMuluQuanxianDialog = self.element.find(".xiugaiMuluQuanxianDialog").eq(0);
        this._xiugaiMuluQuanxianDialog.xiugaiMuluQuanxianDialog({xiugaiHou: function(){
            self._shuaxinGrid();
        }});
        ...
        this.element.find(".btnShanchu").click(function(){
            if(confirm("确实要删除吗?")){
                ...
                $.get("ZhishiGianliController.aspx", {action: "ShanchuMuluQuanxian", muluId: self._muluId, quanxianId: selectedQuanxianId}, function(data){
                    if(data.result == 0){
                        alert(data.message);
                        self._shuaxinGrid();
                    }
    ...
    _shuaxinGrid: function(){
        var self = this;
        $.get("ZhishiGianliController.aspx", {action: "GetMuluQuanxian", muluId: this._muluId}, function(data){
            if(data.result == 0){
                self.element.find(".btnXiugai, .btnShanchu").attr("disabled", "disabled");
    ...
    fenpei: function(muluId, mingcheng){
        var self = this;
        this._muluId = muluId;
        this._shuaxinGrid();
