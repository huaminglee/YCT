...
$.widget( "ui.datagrid", {
	...
	_renderBody: function(){
		...
		if(this.options.data && this.options.data.length){
			...
			this._bodyContent.append("<div style='text-align: center'>无结果</div>");
		}
	},
	...
	_onDatarow_selected: function(row){
		if(this._bodyContent.find(".ui-datagrid-selected").length == this._rows.length){
			this._header.find(".ui-datagrid-header-checkbox-cell input").prop("checked", true);;
		}
	...
	deleteRow: function(deletedRow){
		...
		var deletedRowData = deletedRow.datarow("option", "data");

		var self = this;
		var rows = [];
		$.each(this._rows, function(i, row){
			if(row.is(deletedRow)){
				row.remove();
			}else{;
				rows.push(row);
			}
		...
		self._trigger("deletedRow", null, deletedRowData);
	},
